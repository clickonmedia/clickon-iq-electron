const { BrowserWindow, BrowserView, Menu, ipcMain } = require("electron");
...
class BrowserLikeWindow extends EventEmitter {
  ...
  constructor(options) {
    ...
    channels.forEach(([name, listener]) =>
      ipcMain.on(name, function () {
        ...
        if (typeof listener == "function") listener.apply(null, arguments);
      })
    );
  ...
  setContentBounds() {
    ...
    if (this.currentView) {
      ...
      // this.currentView.setBounds(controlBounds);
    }
  }
  ...
  loadURL(url) {
    ...
    webContents.on(
      "new-window",
      (e, newUrl, frameName, disposition, winOptions) => {
        e.preventDefault();
    ...
    webContents.on("login", (event, request, authInfo, callback) => {
      ...
      callback(
        this.options.proxy.proxyUsername || undefined,
        this.options.proxy.proxyPassword || undefined
      );
    });
    ...
    webContents.on("did-stop-loading", () => {
      ...
      let title =
        this.options.blankTitle == webContents.getTitle()
          ? href
    ...
    webContents.on("did-fail-load", (event, errorCode, errorDescription) => {
      ...
      switch (errorDescription) {
        case "ERR_CONNECTION_TIMED_OUT":
          if (timeout) webContents.loadURL(timeout).then().catch();
          break;
        case "ERR_BLOCKED_BY_CLIENT":
          if (blocked) webContents.loadURL(blocked).then().catch();
          break;
        ...
        case "ERR_CONNECTION_FAILED":
          if (error) webContents.loadURL(error).then().catch();
          break;
      }
    });
